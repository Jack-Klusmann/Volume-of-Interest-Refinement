// code modified from https://github.com/Scrawk/Marching-Cubes/blob/master/Assets/MarchingCubes/Marching/Marching.cs (MIT LICENSE)
...
public abstract class Marching
{
    ...
    public virtual void Generate(float[,,] voxels, IList<Vector3> verts, IList<int> indices, VoxelGridMC voxelGridMC)
    {
        ...
        for (x = 0; x < width - 1; x++)
        {
            for (y = 0; y < height - 1; y++)
            {
                for (z = 0; z < depth - 1; z++)
                {
                    ...
                    //Perform algorithm
                    //March(xStart + x * voxelSize, yStart + y * voxelSize, zStart + z * voxelSize, Cube, verts, indices, _gridSpace);
                    March(x, y, z, Cube, verts, indices,voxelGridMC);
                }
            }
        }
    ...
    /// </summary>
    protected abstract void March(float x, float y, float z, float[] cube, IList<Vector3> vertList, IList<int> indexList, VoxelGridMC voxelGridMC);
